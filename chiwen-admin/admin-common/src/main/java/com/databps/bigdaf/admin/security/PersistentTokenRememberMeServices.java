...
import com.databps.bigdaf.admin.repository.PersistentTokenRepository;
...
/**
 * Custom implementation of Spring Security's RememberMeServices. <p> Persistent tokens are used by
 * Spring Security to automatically log in users. <p> This is a specific implementation of Spring
 * Security's remember-me authentication, but it is much more powerful than the standard
 * implementations: <ul> <li>It allows a user to see the list of his currently opened sessions, and
 * invalidate them</li> <li>It stores more information, such as the IP address and the user agent,
 * for audit purposes<li> <li>When a user logs out, only his current session is invalidated, and not
 * all of his sessions</li> </ul> <p> Please note that it allows the use of the same token for 5
 * seconds, and this value stored in a specific cache during that period. This is to allow
 * concurrent requests from the same user: otherwise, two requests being sent at the same time could
 * invalidate each other's token. <p> This is inspired by: <ul> <li><a
 * href="http://jaspan.com/improved_persistent_login_cookie_best_practice">Improved Persistent Login
 * Cookie Best Practice</a></li> <li><a href="https://github.com/blog/1661-modeling-your-app-s-user-session">GitHub's
 * "Modeling your App's User Session"</a></li> </ul> <p> The main algorithm comes from Spring
 * Security's PersistentTokenBasedRememberMeServices, but this class couldn't be cleanly extended.
 */
@Service
