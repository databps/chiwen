 ...
 */
@Controller
@RequestMapping(value = "/hbase/role")
public class HbaseRoleController extends BaseController {
    @Autowired
   HbaseRoleService  HbaseRoleService;
    ...
    public String listUser(MongoPage page, @RequestParam(required = false) String name, Model model) {
        List<HbaseRoleVo> HbaseRoleVoList = HbaseRoleService.findAllByName(page,name);
        model.addAttribute("HbaseRoleVoList", HbaseRoleVoList);
        model.addAttribute("page", page);
        return "hbase/role/list";
    }

    @RequestMapping(value = "/save", method = RequestMethod.POST)
    public String save(@Valid HbaseRoleVo HbaseRole, @RequestParam(required = false) String roleIdEd) {

        if (StringUtils.isBlank(roleIdEd)) {
            HbaseRoleService.insert(HbaseRole);
        } else {
            HbaseRoleService.update(HbaseRole,roleIdEd);
        }
    ...
    public String delete(@RequestParam(required = false) String roleIdEd) {
        ...
        HbaseRoleService.deleteById(roleIdEd);
    ...
    public String privilege(String[] names,String roleid) {
        if(names==null){
            ...
            HbaseRoleService.modifyGroupPrivilege(names1,roleid);
        }else{
            HbaseRoleService.modifyGroupPrivilege(names,roleid);
        }
        return "redirect:list";
    }
    @ResponseBody
    ...
    public ResponseJson uDetail(@RequestParam(required = true) String roleid) {
        ...
        HbaseRoleVo privilegeForViewVo = HbaseRoleService.findHbasePrivliegeAllByRoleId(null, roleid);
        json.setCode(0);
    ...
    public ResponseJson getPolicy(@RequestParam(required = false) String roleIdEd) {
        ...
        HbaseRole privilege = HbaseRoleService.findOneId(roleIdEd);
    ...
    public Map<String, String> checkLogin(String name, String roleIdEd, Model model) {
       ...
       return HbaseRoleService.checkName(name,roleIdEd);
