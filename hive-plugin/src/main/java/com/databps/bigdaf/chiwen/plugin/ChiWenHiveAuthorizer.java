...
import com.databps.bigdaf.chiwen.util.*;
...
public class ChiWenHiveAuthorizer extends ChiWenHiveAuthorizerBase {
  ...
  private static volatile ChiwenHivePlugin hivePlugin = null;

  public ChiWenHiveAuthorizer(HiveMetastoreClientFactory metastoreClientFactory,
      HiveConf hiveConf,
      HiveAuthenticationProvider hiveAuthenticator,
      HiveAuthzSessionContext sessionContext) {
    super(metastoreClientFactory, hiveConf, hiveAuthenticator, sessionContext);

    LOG.debug("ChiWenHiveAuthorizer.ChiWenHiveAuthorizer()");

    ChiwenHivePlugin plugin = hivePlugin;

    if (plugin == null) {
      synchronized (ChiWenHiveAuthorizer.class) {
        plugin = hivePlugin;
        ...
        if (plugin == null) {
          ...
          plugin = new ChiwenHivePlugin(appType);
          plugin.init();
  ...
  public void grantPrivileges(List<HivePrincipal> hivePrincipals,
    ...
    if (!ChiwenHivePlugin.UpdateXaPoliciesOnGrantRevoke) {
      throw new HiveAuthzPluginException(
          "GRANT/REVOKE not supported in ChiWen HiveAuthorizer. Please use ChiWen Security Admin to setup access control.");
    }
    ChiWenHiveAuditHandler auditHandler = new ChiWenHiveAuditHandler();

    try {
      ChiWenHiveResource resource = getHiveResource(HiveOperationType.GRANT_PRIVILEGE,
          hivePrivObject);
  ...
  public void revokePrivileges(List<HivePrincipal> hivePrincipals,
    ...
    try {
      ChiWenHiveResource resource = getHiveResource(HiveOperationType.REVOKE_PRIVILEGE,
          hivePrivObject);
      GrantRevokeRequest request = createGrantRevokeData(resource, hivePrincipals, hivePrivileges,
          grantorPrincipal, grantOption);
  ...
  public void checkPrivileges(HiveOperationType hiveOpType,
    ...
    try {
      ...
      List<ChiWenHiveAccessRequest> requests = new ArrayList<ChiWenHiveAccessRequest>();

      if (!CollectionUtils.isEmpty(inputHObjs)) {
        for (HivePrivilegeObject hiveObj : inputHObjs) {
          ChiWenHiveResource resource = getHiveResource(hiveOpType, hiveObj);

          if (resource
              == null) { // possible if input object/object is of a kind that we don't currently authorize
            continue;
          }
          ...
          if (hiveObjType == HiveObjectType.URI && isPathInFSScheme(path)) {
            ...
            if (!isURIAccessAllowed(user, permission, path, getHiveConf())) {
              throw new HiveAccessControlException(String
          ...
          if (!existsByResourceAndAccessType(requests, resource, accessType)) {
            ChiWenHiveAccessRequest request = new ChiWenHiveAccessRequest(resource, user, groups,
        ...
        if (hiveOpType == HiveOperationType.SHOWDATABASES) {
          ChiWenHiveResource resource = new ChiWenHiveResource(HiveObjectType.DATABASE, null);
          ChiWenHiveAccessRequest request = new ChiWenHiveAccessRequest(resource, user, groups,
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                "ChiWenHiveAuthorizer.checkPrivileges: Unexpected operation type[" + hiveOpType
      ...
      for (ChiWenHiveAccessRequest request : requests) {
        if (LOG.isDebugEnabled()) {
          LOG.debug("request: " + request);
        }
        ChiWenHiveResource resource = (ChiWenHiveResource) request.getResource();
        ChiWenAccessResult result = null;

        if (resource.getObjectType() == HiveObjectType.COLUMN && StringUtils
            .contains(resource.getColumn(), COLUMN_SEP)) {
          List<ChiWenAccessRequest> colRequests = new ArrayList<ChiWenAccessRequest>();
          ...
          for (String column : columns) {
            ...
            ChiWenHiveResource colResource = new ChiWenHiveResource(HiveObjectType.COLUMN,
                resource.getDatabase(), resource.getTable(), column);

            ChiWenHiveAccessRequest colRequest = request.copy();
            colRequest.setResource(colResource);

            colRequests.add(colRequest);
          }

          Collection<ChiWenAccessResult> colResults = hivePlugin
              .isAccessAllowed(colRequests, auditHandler);
          if (colResults != null) {
            for (ChiWenAccessResult colResult : colResults) {
              result = colResult;
        ...
        if ((result == null || result.getIsAllowed())
          ...
          ChiWenHiveResource tblResource = new ChiWenHiveResource(HiveObjectType.TABLE,
              resource.getDatabase(), resource.getTable());

          request.setHiveAccessType(
              HiveAccessType.SELECT); // filtering/masking policies are defined only for SELECT
          ...
          request.setResourceMatchingScope(
              ChiWenAccessRequest.ResourceMatchingScope.SELF_OR_DESCENDANTS);
        ...
        if (result == null || !result.getIsAllowed()) {
          ...
          throw new HiveAccessControlException(
              String.format("Permission denied: user [%s] does not have [%s] privilege on [%s]",
  ...
  public List<HivePrivilegeObject> filterListCmdObjects(List<HivePrivilegeObject> objs,
    ...
    if (objs == null) {
      ...
      if (ret
          == null) { // if we got any items to filter then we can't return back a null.  We must return back a list even if its empty.
      ...
      for (HivePrivilegeObject privilegeObject : objs) {
        if (LOG.isDebugEnabled()) {
          ...
          LOG.debug(String
              .format(format, actionType, objectType, objectName, dbName, columns, partitionKeys,
                  commandString, ipAddress));
        }

        ChiWenHiveResource resource = createHiveResource(privilegeObject);
        if (resource == null) {
          LOG.error(
              "filterListCmdObjects: ChiWenHiveResource returned by createHiveResource is null");
        } else {
          ChiWenHiveAccessRequest request = new ChiWenHiveAccessRequest(resource, user, groups,
              context, sessionContext, hivePlugin.getClusterName());
          ChiWenAccessResult result = hivePlugin.isAccessAllowed(request);
          if (result == null) {
            LOG.error(
                "filterListCmdObjects: Internal error: null ChiWenAccessResult object received back from isAccessAllowed()!");
  ...
  ChiWenHiveResource createHiveResource(HivePrivilegeObject privilegeObject) {
    ChiWenHiveResource resource = null;
    ...
    switch (objectType) {
      case DATABASE:
        resource = new ChiWenHiveResource(HiveObjectType.DATABASE, objectName);
        break;
      case TABLE_OR_VIEW:
        resource = new ChiWenHiveResource(HiveObjectType.TABLE, dbName, objectName);
        break;
      default:
        LOG.warn("ChiWenHiveAuthorizer.getHiveResource: unexpected objectType:" + objectType);
    }
  ...
  private ChiWenHiveResource getHiveResource(HiveOperationType hiveOpType,
      HivePrivilegeObject hiveObj) {
    ChiWenHiveResource ret = null;

    HiveObjectType objectType = getObjectType(hiveObj, hiveOpType);

    switch (objectType) {
      case DATABASE:
        ret = new ChiWenHiveResource(objectType, hiveObj.getDbname());
        break;
      ...
      case INDEX:
      case FUNCTION:
        ret = new ChiWenHiveResource(objectType, hiveObj.getDbname(), hiveObj.getObjectName());
        break;
      ...
      case URI:
        ret = new ChiWenHiveResource(objectType, hiveObj.getObjectName());
        break;
      ...
      case OTHER:
        switch (hiveOpType) {
          ...
          case DESCTABLE:
            switch (StringUtil.toLower(hivePlugin.DescribeShowTableAuth)) {
              case "show-allowed":
                ...
                // SELECT/SHOWCOLUMS/DESCTABLE to filter the columns based on access provided in ChiWen.
              case "none":
              case "":
                accessType = HiveAccessType.SELECT;
                break;
          ...
          case REVOKE_PRIVILEGE:
            accessType = HiveAccessType.NONE; // access check will be performed at the ChiWen-admin side
            break;
    ...
    switch (hiveOpType) {
      ...
      case QUERY:
        try {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                "ChiWenHiveAuthorizer.buildPathForException(): Error while creating exception message ",
                e);
          }
        }
        break;
  ...
  private void handleDfsCommand(HiveOperationType hiveOpType,
      List<HivePrivilegeObject> inputHObjs,
      String user,
      ChiWenHiveAuditHandler auditHandler)
      throws HiveAuthzPluginException, HiveAccessControlException {
    ...
    if (hivePlugin != null) {
      serviceId=hivePlugin.getServiceId();
    }

    auditHandler.logAuditEventForDfs(user, dfsCommandParams, false, serviceId);

    throw new HiveAccessControlException(
        String.format("Permission denied: user [%s] does not have privilege for [%s] command",
            user, hiveOpType.name()));
  }

  private boolean existsByResourceAndAccessType(Collection<ChiWenHiveAccessRequest> requests,
      ChiWenHiveResource resource, HiveAccessType accessType) {
    boolean ret = false;
  ...
  private GrantRevokeRequest createGrantRevokeData(ChiWenHiveResource resource,
    ...
    if (resource == null ||
      ...
      throw new HiveAccessControlException(
          "grant/revoke: unexpected object type '" + (resource == null ? null
    ...
    mapResource.put(ChiWenHiveResource.KEY_DATABASE, database);
    mapResource.put(ChiWenHiveResource.KEY_TABLE, table);
    mapResource.put(ChiWenHiveResource.KEY_COLUMN, column);
    ...
    HiveAuthzSessionContext sessionContext = getHiveAuthzSessionContext();
    if (sessionContext != null) {
      ret.setClientType(sessionContext.getClientType() == null ? null
          : sessionContext.getClientType().toString());
    }
  ...
  public List<HivePrivilegeInfo> showPrivileges(HivePrincipal principal,
    ...
    try {
      ...
      LOG.debug("ChiWenHiveAuthorizer.showPrivileges()");
      ...
      if (msObjPrivs != null) {
        ...
        throw new HiveAccessControlException(
            "ChiWenHiveAuthorizer.showPrivileges():User has to specify"
      ...
      LOG.error(
          "ChiWenHiveAuthorizer.showPrivileges: showPrivileges returned by showPrivileges is null");
  ...
  private boolean isBlockAccessIfRowfilterColumnMaskSpecified(HiveOperationType hiveOpType,
      ChiWenHiveAccessRequest request) {
    boolean ret = false;
    ChiWenHiveResource resource = (ChiWenHiveResource) request.getResource();
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("isBlockAccessIfRowfilterColumnMaskSpecified(" + hiveOpType + ", " + request + "): "
          + ret);
    }
  ...
  private String toString(HiveOperationType hiveOpType,
    ...
    sb.append("'clientType':")
        .append(sessionContext == null ? null : sessionContext.getClientType());
    ...
    sb.append(", 'sessionString':")
        .append(sessionContext == null ? "null" : sessionContext.getSessionString());
    sb.append("}");

    sb.append(", 'user':").append(this.getCurrentUserGroupInfo().getUserName());
    sb.append(", 'groups':[")
        .append(StringUtil.toString(this.getCurrentUserGroupInfo().getGroupNames())).append("]");
  ...
  private StringBuilder toString(HivePrivilegeObject privObj, StringBuilder sb) {
    ...
    sb.append(", 'commandParams':[").append(StringUtil.toString(privObj.getCommandParams()))
        .append("]");
...
class ChiwenHivePlugin extends ChiWenBasePlugin {

  public static boolean UpdateXaPoliciesOnGrantRevoke = ChiWenHadoopConstants.HIVE_UPDATE_ChiWen_POLICIES_ON_GRANT_REVOKE_DEFAULT_VALUE;
  public static boolean BlockUpdateIfRowfilterColumnMaskSpecified = ChiWenHadoopConstants.HIVE_BLOCK_UPDATE_IF_ROWFILTER_COLUMNMASK_SPECIFIED_DEFAULT_VALUE;
  public static String DescribeShowTableAuth = ChiWenHadoopConstants.HIVE_DESCRIBE_TABLE_SHOW_COLUMNS_AUTH_OPTION_PROP_DEFAULT_VALUE;

  private static String ChiWen_PLUGIN_HIVE_ULRAUTH_FILESYSTEM_SCHEMES = "ChiWen.plugin.hive.urlauth.filesystem.schemes";
  private static String ChiWen_PLUGIN_HIVE_ULRAUTH_FILESYSTEM_SCHEMES_DEFAULT = "hdfs:,file:";
  private static String FILESYSTEM_SCHEMES_SEPARATOR_CHAR = ",";
  private String[] fsScheme = null;

  public ChiwenHivePlugin(String appType) {
    super("hive", appType, "hive");
  }
  ...
  public void init() {
    ...
    ChiwenHivePlugin.UpdateXaPoliciesOnGrantRevoke = ChiWenConfiguration.getInstance()
        .getBoolean(ChiWenHadoopConstants.HIVE_UPDATE_ChiWen_POLICIES_ON_GRANT_REVOKE_PROP,
            ChiWenHadoopConstants.HIVE_UPDATE_ChiWen_POLICIES_ON_GRANT_REVOKE_DEFAULT_VALUE);
    ChiwenHivePlugin.BlockUpdateIfRowfilterColumnMaskSpecified = ChiWenConfiguration.getInstance()
        .getBoolean(ChiWenHadoopConstants.HIVE_BLOCK_UPDATE_IF_ROWFILTER_COLUMNMASK_SPECIFIED_PROP,
            ChiWenHadoopConstants.HIVE_BLOCK_UPDATE_IF_ROWFILTER_COLUMNMASK_SPECIFIED_DEFAULT_VALUE);
    ChiwenHivePlugin.DescribeShowTableAuth = ChiWenConfiguration.getInstance()
        .get(ChiWenHadoopConstants.HIVE_DESCRIBE_TABLE_SHOW_COLUMNS_AUTH_OPTION_PROP,
            ChiWenHadoopConstants.HIVE_DESCRIBE_TABLE_SHOW_COLUMNS_AUTH_OPTION_PROP_DEFAULT_VALUE);

    String fsSchemesString = ChiWenConfiguration.getInstance()
        .get(ChiWen_PLUGIN_HIVE_ULRAUTH_FILESYSTEM_SCHEMES,
            ChiWen_PLUGIN_HIVE_ULRAUTH_FILESYSTEM_SCHEMES_DEFAULT);
