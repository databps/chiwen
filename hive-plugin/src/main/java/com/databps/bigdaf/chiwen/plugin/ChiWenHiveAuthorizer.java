...
public class ChiWenHiveAuthorizer extends ChiWenHiveAuthorizerBase {
	...
	public void grantPrivileges(List<HivePrincipal> hivePrincipals,
		...
		if(! ChiwenHivePlugin.UpdateXaPoliciesOnGrantRevoke) {
			throw new HiveAuthzPluginException("GRANT/REVOKE not supported in ChiWen HiveAuthorizer. Please use ChiWen Security Admin to setup access control.");
		}
		ChiWenHiveAuditHandler auditHandler = new ChiWenHiveAuditHandler();

		try {
			ChiWenHiveResource resource = getHiveResource(HiveOperationType.GRANT_PRIVILEGE, hivePrivObject);
	...
	public void revokePrivileges(List<HivePrincipal> hivePrincipals,
		...
		try {
			ChiWenHiveResource resource = getHiveResource(HiveOperationType.REVOKE_PRIVILEGE, hivePrivObject);
			GrantRevokeRequest request  = createGrantRevokeData(resource, hivePrincipals, hivePrivileges, grantorPrincipal, grantOption);
			...
			if(!CollectionUtils.isEmpty(inputHObjs)) {
				for(HivePrivilegeObject hiveObj : inputHObjs) {
					...
					if(!existsByResourceAndAccessType(requests, resource, accessType)) {
						ChiWenHiveAccessRequest request = new ChiWenHiveAccessRequest(resource, user, groups, hiveOpType, accessType, context, sessionContext);
				...
				if (hiveOpType == HiveOperationType.SHOWDATABASES) {
					...
					ChiWenHiveAccessRequest request = new ChiWenHiveAccessRequest(resource, user, groups, hiveOpType.name(), HiveAccessType.USE, context, sessionContext);
					requests.add(request);
				} else {
					if (LOG.isDebugEnabled()) {
						LOG.debug("ChiWenHiveAuthorizer.checkPrivileges: Unexpected operation type[" + hiveOpType + "] received with empty input objects list!");
					}
				}
			}
			...
			for(ChiWenHiveAccessRequest request : requests) {
				...
				if(resource.getObjectType() == HiveObjectType.COLUMN && StringUtils.contains(resource.getColumn(), COLUMN_SEP)) {
					...
					for(String column : columns) {
						...
						ChiWenHiveResource colResource = new ChiWenHiveResource(HiveObjectType.COLUMN, resource.getDatabase(), resource.getTable(), column);
					...
					Collection<ChiWenAccessResult> colResults = hivePlugin.isAccessAllowed(colRequests, auditHandler);
					if(colResults != null) {
				...
				if((result == null || result.getIsAllowed()) && isBlockAccessIfRowfilterColumnMaskSpecified(hiveOpType, request)) {
					...
					ChiWenHiveResource tblResource     = new ChiWenHiveResource(HiveObjectType.TABLE, resource.getDatabase(), resource.getTable());
					...
					request.setResourceMatchingScope(ChiWenAccessRequest.ResourceMatchingScope.SELF_OR_DESCENDANTS);
	...
	public List<HivePrivilegeObject> filterListCmdObjects(List<HivePrivilegeObject> objs,
		...
		if (objs == null) {
			...
			for (HivePrivilegeObject privilegeObject : objs) {
				...
				if (resource == null) {
					LOG.error("filterListCmdObjects: ChiWenHiveResource returned by createHiveResource is null");
				} else {
	...
	private ChiWenHiveResource getHiveResource(HiveOperationType hiveOpType,
		...
		switch(objectType) {
			...
			case COLUMN:
				ret = new ChiWenHiveResource(objectType, hiveObj.getDbname(), hiveObj.getObjectName(), StringUtils.join(hiveObj.getColumns(), COLUMN_SEP));
			break;
		...
		switch(hiveOpType) {
			...
			case QUERY:
				try {
					...
					if(LOG.isDebugEnabled()) {
						LOG.debug("ChiWenHiveAuthorizer.buildPathForException(): Error while creating exception message ", e);
					}
				}
				break;
		}
	...
	private void handleDfsCommand(HiveOperationType hiveOpType,
								  List<HivePrivilegeObject> inputHObjs,
								  String                    user,
								  ChiWenDefaultAuditHandler    auditHandler)
	      throws HiveAuthzPluginException, HiveAccessControlException {
		...
		//ditHandler.logAuditEventForDfs(user, dfsCommandParams, false, serviceType, serviceName);
	...
	private boolean existsByResourceAndAccessType(Collection<ChiWenHiveAccessRequest> requests, ChiWenHiveResource resource, HiveAccessType accessType) {
		boolean ret = false;
	...
	public List<HivePrivilegeInfo> showPrivileges(HivePrincipal principal,
		...
		try {
			...
			LOG.error("ChiWenHiveAuthorizer.showPrivileges: showPrivileges returned by showPrivileges is null");
	...
	private boolean isBlockAccessIfRowfilterColumnMaskSpecified(HiveOperationType hiveOpType, ChiWenHiveAccessRequest request) {
		boolean            ret      = false;
		ChiWenHiveResource resource = (ChiWenHiveResource)request.getResource();
...
class ChiwenHivePlugin extends ChiWenBasePlugin {
	...
	public void init() {
		...
		ChiwenHivePlugin.UpdateXaPoliciesOnGrantRevoke = ChiWenConfiguration.getInstance().getBoolean(ChiWenHadoopConstants.HIVE_UPDATE_ChiWen_POLICIES_ON_GRANT_REVOKE_PROP, ChiWenHadoopConstants.HIVE_UPDATE_ChiWen_POLICIES_ON_GRANT_REVOKE_DEFAULT_VALUE);
		ChiwenHivePlugin.BlockUpdateIfRowfilterColumnMaskSpecified = ChiWenConfiguration.getInstance().getBoolean(ChiWenHadoopConstants.HIVE_BLOCK_UPDATE_IF_ROWFILTER_COLUMNMASK_SPECIFIED_PROP, ChiWenHadoopConstants.HIVE_BLOCK_UPDATE_IF_ROWFILTER_COLUMNMASK_SPECIFIED_DEFAULT_VALUE);
		ChiwenHivePlugin.DescribeShowTableAuth = ChiWenConfiguration.getInstance().get(ChiWenHadoopConstants.HIVE_DESCRIBE_TABLE_SHOW_COLUMNS_AUTH_OPTION_PROP, ChiWenHadoopConstants.HIVE_DESCRIBE_TABLE_SHOW_COLUMNS_AUTH_OPTION_PROP_DEFAULT_VALUE);

		String fsSchemesString = ChiWenConfiguration.getInstance().get(ChiWen_PLUGIN_HIVE_ULRAUTH_FILESYSTEM_SCHEMES, ChiWen_PLUGIN_HIVE_ULRAUTH_FILESYSTEM_SCHEMES_DEFAULT);
