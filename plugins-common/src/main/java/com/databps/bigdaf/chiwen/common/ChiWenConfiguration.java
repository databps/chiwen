...
import com.databps.bigdaf.chiwen.audit.AuditProviderFactory;
import java.net.URL;
...
public class ChiWenConfiguration extends Configuration {
	private static final Logger LOG = Logger.getLogger(ChiWenConfiguration.class);
	
	private static volatile ChiWenConfiguration config;
	...
	public static ChiWenConfiguration getInstance() {
        ChiWenConfiguration result = config;
		if (result == null) {
			synchronized (ChiWenConfiguration.class) {
				result = config;
				if (result == null) {
					config = result = new ChiWenConfiguration();
				}
			}
		}
		return result;
...
//		}
	...
	private void  addAuditResource(String serviceType) {
		...
		URL url = null;
		try {
			url = ChiWenLegacyConfigBuilder.getAuditConfig(serviceType);
