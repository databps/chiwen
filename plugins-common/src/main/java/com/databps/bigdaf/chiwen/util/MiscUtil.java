...
//    logger.info("SUBJECT " + (subject == null ? "not found" : "found"));
  ...
  public static UserGroupInformation getUGILoginUser() {
    ...
    if (ret != null) {
      try {
        ...
        logger.error(
            "Error renewing TGT and relogin. Ignoring Exception, and continuing with the old TGT",
            ioe);
      }
    }
  ...
  /**
   * Execute the {@link PrivilegedExceptionAction} on the {@link UserGroupInformation} if it's set,
  ...
  /**
   * Execute the {@link PrivilegedAction} on the {@link UserGroupInformation} if it's set, otherwise
   * call it directly.
   */
  ...
  public static void setUGIFromJAASConfig(String jaasConfigAppName) throws Exception {
    ...
    try {
      AppConfigurationEntry entries[] = Configuration.getConfiguration()
          .getAppConfigurationEntry(jaasConfigAppName);
      if (!ArrayUtils.isEmpty(entries)) {
        ...
        if (!StringUtils.isEmpty(principal) && !StringUtils.isEmpty(keytabFile)) {
          ...
          String error_mesage =
              "Unable to get the principal/keytab from jaasConfigAppName: " + jaasConfigAppName;
          logger.error(error_mesage);
          throw new Exception(error_mesage);
        }
        logger.info(
            "MiscUtil.setUGIFromJAASConfig() UGI: " + ugi + " principal: " + principal + " keytab: "
                + keytabFile);
      } else {
        logger.warn(
            "JAASConfig file not found! ChiWen Plugin will not working in a Secure Cluster...");
      }
    } catch (Exception e) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "<=== MiscUtil.setUGIFromJAASConfig() jaasConfigAppName: " + jaasConfigAppName + " UGI: "
              + ugi + " principal: " + principal + " keytab: " + keytabFile);
    }
  }
